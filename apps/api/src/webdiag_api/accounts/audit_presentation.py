...
_CHECK_NAMES_RU: dict[str, str] = {
    ...
    "performance.pagespeed_mobile": "Производительность PageSpeed на мобильных",
}
...
_ISSUES_RU: dict[str, _IssuePresentation] = {
    ...
    "performance.pagespeed_mobile.poor": _IssuePresentation(
        "Низкая производительность PageSpeed на мобильных",
        "Google PageSpeed зафиксировал низкую мобильную производительность или хотя бы одну метрику с неудовлетворительным результатом.",
        "Сначала устраните наиболее значимые проблемы производительности из PageSpeed.",
        (
            "Проверьте LCP, CLS, INP, FCP, TBT и Speed Index и начните с метрик со статусом fail.",
            "Исправьте самые весомые рекомендации Lighthouse и повторите мобильную проверку.",
        ),
        "Снижает задержки загрузки и взаимодействия на мобильных устройствах и улучшает пользовательские сигналы производительности.",
    ),
    "performance.pagespeed_mobile.needs_improvement": _IssuePresentation(
        "Производительность PageSpeed на мобильных требует улучшения",
        "Google PageSpeed показал результат ниже хорошего диапазона или одну из метрик в зоне, требующей улучшения.",
        "Последовательно устраните основные рекомендации PageSpeed и повторите аудит.",
        (
            "Начните с метрик и возможностей с наибольшим потенциальным эффектом.",
            "После изменений повторите мобильный PageSpeed-аудит и сравните показатели.",
        ),
        "Помогает приблизить мобильные метрики производительности к хорошему диапазону без завышенных обещаний по поисковому ранжированию.",
    ),
}
