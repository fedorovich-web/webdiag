...
from webdiag_api.ai.artifacts import ArtifactStorage, StoredArtifact
from webdiag_api.ai.images import MAX_IMAGE_BYTES
...
def artifact_storage_from_env(
    ...
    *,
    ...
    if backend == "local":
        ...
        root = _required(values, "WEBDIAG_AI_ARTIFACT_LOCAL_ROOT")
    ...
    region = _required(values, "WEBDIAG_AI_ARTIFACT_S3_REGION")
    access_key = _required(values, "WEBDIAG_AI_ARTIFACT_S3_ACCESS_KEY_ID")
    secret_key = _required(values, "WEBDIAG_AI_ARTIFACT_S3_SECRET_ACCESS_KEY")
    ...
    client = factory("s3", **client_options)
...
def _validate_max_bytes(max_bytes: int) -> None:
    if max_bytes < 1 or max_bytes > MAX_IMAGE_BYTES:
