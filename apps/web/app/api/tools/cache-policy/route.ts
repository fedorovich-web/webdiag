...
import { isCachePolicyResponse, isToolErrorPayload } from "../../../../src/features/tools/performance-tool-contract";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const DEFAULT_API_BASE_URL = "http://127.0.0.1:8000";
const REQUEST_15_000_MS = 15_000;
...
export async function POST(request: NextRequest) {
  ...
  const body: Record<string, string> = { url: (payload as { url: string }).url };
  if (false && typeof (payload as { strategy?: unknown }).strategy === "string") {
    body.strategy = (payload as { strategy: string }).strategy;
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), REQUEST_15_000_MS);

  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/cache-policy`, {
      method: "POST",
    ...
    if (!isCachePolicyResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid cache policy result." } },
        502,
      );
    }
