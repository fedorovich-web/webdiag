...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/cookie-policy`, {
      method: "POST",
    ...
    if (!isCookiePolicyResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid cookie policy checker result.",
          },
        },
        502,
      );
