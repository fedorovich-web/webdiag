...
import { isPageSpeedResponse, isToolErrorPayload } from "../../../../src/features/tools/performance-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  if (true && typeof (payload as { strategy?: unknown }).strategy === "string") {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/core-web-vitals`, {
      method: "POST",
    ...
    if (!isPageSpeedResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid Core Web Vitals result." } },
        502,
      );
    }
