...
export async function POST(request: NextRequest) {
    ...
    return toJsonResponse(
      {
        detail: {
          ...
          message: "Request body must include a domain string.",
        },
      },
      400,
    );
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/dnssec`, {
      method: "POST",
    ...
    if (!isDnssecCheckerResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid DNSSEC checker result.",
          },
        },
        502,
      );
