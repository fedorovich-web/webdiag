...
import { isHtmlMarkupValidatorResponse, isToolErrorPayload } from "../../../../src/features/tools/markup-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/html-validator`, {
      method: "POST",
    ...
    if (!isHtmlMarkupValidatorResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid HTML markup result." } },
        502,
      );
    }
