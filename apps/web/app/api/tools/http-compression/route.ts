...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/http-compression`, {
      method: "POST",
    ...
    if (!isHttpCompressionResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid HTTP compression checker result.",
          },
        },
        502,
      );
