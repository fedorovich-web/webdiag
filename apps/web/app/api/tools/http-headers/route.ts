...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/http-headers`, {
      method: "POST",
    ...
    if (!isHttpHeadersAnalyzerResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid HTTP headers analyzer result.",
          },
        },
        502,
      );
