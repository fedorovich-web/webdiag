...
import { isToolErrorPayload, isMetaTagsResponse } from "../../../../src/features/tools/metadata-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/meta-tags`, {
      method: "POST",
    ...
    if (!isMetaTagsResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid meta tags result." } },
        502,
      );
    }
