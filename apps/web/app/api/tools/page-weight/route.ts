...
import { isPageWeightResponse, isToolErrorPayload } from "../../../../src/features/tools/performance-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/page-weight`, {
      method: "POST",
    ...
    if (!isPageWeightResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid page weight result." } },
        502,
      );
    }
