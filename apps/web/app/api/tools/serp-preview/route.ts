...
import { isToolErrorPayload, isSerpPreviewResponse } from "../../../../src/features/tools/metadata-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/serp-preview`, {
      method: "POST",
    ...
    if (!isSerpPreviewResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid SERP preview result." } },
        502,
      );
    }
