...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/server-timing`, {
      method: "POST",
    ...
    if (!isServerTimingAnalyzerResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid server timing analyzer result.",
          },
        },
        502,
      );
