...
import { isToolErrorPayload, isSocialPreviewResponse } from "../../../../src/features/tools/metadata-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/social-preview`, {
      method: "POST",
    ...
    if (!isSocialPreviewResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid social preview result." } },
        502,
      );
    }
