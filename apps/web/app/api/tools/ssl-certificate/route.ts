...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/ssl-certificate`, {
      method: "POST",
    ...
    if (!isSslCertificateResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid SSL certificate checker result.",
          },
        },
        502,
      );
