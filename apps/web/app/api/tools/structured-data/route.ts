...
import { isStructuredDataValidatorResponse, isToolErrorPayload } from "../../../../src/features/tools/markup-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/structured-data`, {
      method: "POST",
    ...
    if (!isStructuredDataValidatorResponse(data)) {
      return toJsonResponse(
        { detail: { code: "tool_api_invalid_response", message: "Tool API returned an invalid structured data result." } },
        502,
      );
    }
