...
import {
  ...
  isToolErrorPayload,
} from "../../../../src/features/tools/technical-seo-tool-contract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${getApiBaseUrl()}/v1/tools/technology-detector`, {
      method: "POST",
    ...
    if (!isTechnologyDetectorResponse(data)) {
      return toJsonResponse(
        {
          detail: {
            code: "tool_api_invalid_response",
            message: "Tool API returned an invalid technology detector result.",
          },
        },
        502,
      );
