...
export function AccountSavedAudit({ locale, projectId, auditId }: { readonly locale: Locale; readonly projectId: string; readonly auditId: string }) {
  ...
  return (
    <section className="wd-account-dashboard">
      ...
      <section className="wd-saved-checks" aria-labelledby="saved-checks-title"><h2 id="saved-checks-title">{ru ? "Проверки" : "Checks"}</h2><div className="wd-check-grid">{detail.payload.checks.map((check) => <article key={check.check_id}><h3>{check.name}</h3><p>{reportCheckStatusLabel(locale, check.status)}</p></article>)}</div></section>
