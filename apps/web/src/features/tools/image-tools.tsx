...
class UnsupportedCanvasEncoderError extends Error {
  readonly format: RasterOutputFormat;

  constructor(format: RasterOutputFormat) {
    super(`The browser could not encode ${formatMimeLabel(format)}.`);
    this.name = "UnsupportedCanvasEncoderError";
    this.format = format;
  }
}

export function assertEncodedBlobFormat(blob: Blob, format: RasterOutputFormat): Blob {
  if (blob.type.toLowerCase() !== format) {
    throw new UnsupportedCanvasEncoderError(format);
  }
  return blob;
}
...
function canvasToBlob(canvas: HTMLCanvasElement, format: RasterOutputFormat, quality: number): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (!blob) {
          reject(new UnsupportedCanvasEncoderError(format));
          return;
        }
        try {
          resolve(assertEncodedBlobFormat(blob, format));
        } catch (error) {
          reject(error);
        }
      },
...
function imageProcessingError(
  ...
  if (error instanceof UnsupportedCanvasEncoderError) {
    const format = formatMimeLabel(error.format);
    return t(
      locale,
      `Этот браузер не поддерживает кодирование ${format}. Используйте WebP, JPEG или PNG.`,
      `This browser cannot encode ${format}. Use WebP, JPEG, or PNG.`,
    );
  }
  return error instanceof Error ? error.message : t(locale, fallbackRu, fallbackEn);
}
...
export function ImageOptimizerTool({ locale }: { locale: Locale }) {
  ...
  async function run() {
    ...
    try {
      ...
      setError("");
    } catch (caught) { setError(imageProcessingError(caught, locale, "Не удалось обработать изображение.", "Could not process the image.")); }
  }
...
export function ImageFormatConverterTool({ locale }: { locale: Locale }) {
  ...
  async function run() {
    ...
    try {
      ...
      setError("");
    } catch (caught) { setError(imageProcessingError(caught, locale, "Не удалось конвертировать изображение.", "Could not convert the image.")); }
  }
...
export function ImageResizerTool({ locale }: { locale: Locale }) {
  ...
  async function run() {
    ...
    try {
      ...
      setError("");
    } catch (caught) { setError(imageProcessingError(caught, locale, "Проверьте размеры.", "Check the dimensions.")); }
  }
...
export function ImageCropperTool({ locale }: { locale: Locale }) {
  ...
  async function run() {
    ...
    try {
      ...
      setError("");
    } catch (caught) { setError(imageProcessingError(caught, locale, "Проверьте область обрезки.", "Check the crop area.")); }
  }
