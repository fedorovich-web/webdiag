...
.DESCRIPTION
  Run from the WebDiag project root. Collects source files, configs, tests,
  docs and generated git snapshot into a zip.

  Excludes node_modules, .next, dist/build/out, .venv, caches, .env,
  old archives and font binaries (*.woff, *.woff2, *.ttf, *.otf).

  Font exclusion is intentional: do not upload/share font files in ChatGPT.
#>
...
function Test-ExcludedPath([string]$RelativePath) {
  ...
  $excludedDirs = @(
    ...
    ".mypy_cache/", ".ruff_cache/", "coverage/", "test-results/",
    "playwright-report/", ".git/", ".turbo/", ".cache/"
  )

  foreach ($dir in $excludedDirs) {
    if ($n.StartsWith($dir) -or $n.Contains("/$dir")) { return $true }
  }

  $fileName = Split-Path $n -Leaf
  $excludedNames = @(".env", ".env.local", ".env.production", ".env.development", ".DS_Store", "Thumbs.db")
  if ($excludedNames -contains $fileName) { return $true }

  $excludedExtensions = @(".zip", ".7z", ".rar", ".tar", ".gz", ".log", ".tmp", ".woff", ".woff2", ".ttf", ".otf")
  foreach ($ext in $excludedExtensions) {
    if ($fileName.ToLowerInvariant().EndsWith($ext)) { return $true }
  }
...
function Copy-RelativeFile([string]$SourceFile, [string]$Root, [string]$DestRoot) {
  $rel = [System.IO.Path]::GetRelativePath($Root, $SourceFile)
...
if (Test-Path $packageJsonPath) {
  try {
    ...
    Write-Info "Could not parse package.json: $($_.Exception.Message)"
  }
}
...
$rootFiles = @(
  ...
  "npm-shrinkwrap.json", "tsconfig.json", "tsconfig.base.json",
  "eslint.config.js", "eslint.config.mjs", "vitest.config.ts",
  "vitest.config.js", "playwright.config.ts", "playwright.config.js",
  "next.config.js", "next.config.mjs", "next.config.ts",
  "pyproject.toml", "ruff.toml", "README.md", "CHANGELOG.md",
...
$includeDirs = @("HANDOFF", "apps", "packages", "scripts", "tests", "e2e", "docs", ".github")
...
if ($insideGit) {
  ...
  try {
    ...
    if (-not $NoGitDiff) {
      git diff -- . ":(exclude).env" ":(exclude).env.local" ":(exclude)*.woff" ":(exclude)*.woff2" ":(exclude)*.ttf" ":(exclude)*.otf" |
        Set-Content -LiteralPath (Join-Path $handoffDir "GIT_DIFF.patch") -Encoding UTF8
    }
  } finally {
    Pop-Location
  }
} else {
  Set-Content -LiteralPath $gitSnapshotPath -Value "# Git Snapshot`n`nNo .git repository detected. Do not infer branch, HEAD, origin or status." -Encoding UTF8
}
...
foreach ($file in $files) {
  $rel = [System.IO.Path]::GetRelativePath($stageRoot, $file.FullName) -replace "\\", "/"
...
Write-Info "Files included before final generated manifest/hash files: $($files.Count)"
